...
var (
	...
	QueryLinkPurposeToGroup = dbmodel.DBQuery{
		...
		Query: "INSERT INTO CONSENT_PURPOSE_ELEMENT_MAPPING (GROUP_ID, ELEMENT_ID, IS_MANDATORY, ORG_ID) VALUES (?, ?, ?, ?)",
	}

	QueryGetGroupPurposes = dbmodel.DBQuery{
		ID: "GET_GROUP_PURPOSES",
		Query: `SELECT m.ELEMENT_ID as PURPOSE_ID, p.NAME as PURPOSE_NAME, m.IS_MANDATORY 
				FROM CONSENT_PURPOSE_ELEMENT_MAPPING m 
	...
	QueryDeleteGroupPurposes = dbmodel.DBQuery{
		...
		Query: "DELETE FROM CONSENT_PURPOSE_ELEMENT_MAPPING WHERE GROUP_ID = ? AND ORG_ID = ?",
	}
	...
	queryCheckPurposeInGroups = dbmodel.DBQuery{
		...
		Query: "SELECT COUNT(*) as count FROM CONSENT_PURPOSE_ELEMENT_MAPPING WHERE ELEMENT_ID = ? AND ORG_ID = ?",
	}
)
...
func (s *store) ListGroups(ctx context.Context, orgID, name string, clientIDs []string, purposeNames []string, offset, limit int) ([]model.PurposeGroup, int, error) {
	...
	if len(purposeNames) > 0 {
		...
		for _, purposeName := range purposeNames {
			subQuery := ` AND EXISTS (
				SELECT 1 FROM CONSENT_PURPOSE_ELEMENT_MAPPING m
				JOIN CONSENT_ELEMENT p ON m.ELEMENT_ID = p.ID AND m.ORG_ID = p.ORG_ID
